...
RESPONSE_EXAMPLES = {
    "/login": dict(userId="user123", authToken="123456"),
    "/logout": dict(),
    "/files": dict(status="success", data=dict(jobId=STR_EXAMPLE)),
    ...
    "/modeloutput/{id}/files": dict(status="success", data=[STR_EXAMPLE]),
...
def test_get_analysis_status(client, mocker):
    ...
    with mocker as rm:
        ...
        response = client.get_analysis_status(
            **REQUEST_EXAMPLES["/modeloutput/{id}/status"]
        )
        assert response.status_code == 200
